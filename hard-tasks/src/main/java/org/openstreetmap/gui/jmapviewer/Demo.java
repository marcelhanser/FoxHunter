...
public class Demo extends JFrame implements JMapViewerEventListener {
	...
	public Demo() {
		...
		mperpLabelValue = new JLabel(
				String.format("%s", map.getMeterPerPixel()));
		...
		JComboBox tileSourceSelector = new JComboBox(new TileSource[] {
				...
				new BingAerialTileSource(), new MapQuestOsmTileSource(),
		...
		//
		map.addMapMarker(new MapMarkerDot(Color.BLACK, 49.814284999,
				8.642065999));
		...
		map.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getSource() == map) {
					...
					System.out.println(String.format("mouse clicked %d, %d",
							x2, y2));

					Coordinate position = map.getPosition(e.getPoint());

					map.addMapMarker(new MapMarkerDot(position.getLat(),
							position.getLon()));
				}
				// e.get
			}
	...
	private void updateZoomParameters() {
		if (mperpLabelValue != null)
			mperpLabelValue
					.setText(String.format("%s", map.getMeterPerPixel()));
		if (zoomValue != null)
